...
@Autonomous(name = "Autonomous Blue [NS]", group = "Autonomous")
@Disabled
public class AutoBotBlueNoSquareLinear extends OpModeCamera {
    ...
    public void loop() {
        ...
        switch (state) {
            case STATE_FIRST: // Initialization
                ...
                s2.setPosition(0.38); // Lower gate.
            ...
            case STATE_FORWARDS_BEFORE_ARC: // Going forwards.
                if ((getLeftPos() >= 325 * Constants.ENCODER_MULT) && (getRightPos() >= 325 * Constants.ENCODER_MULT)) {
                    resetEncoders();
                    left.setPower(-0.3 / LEFT_SPEED_FACTOR);
                    right.setPower(-0.3);
            ...
            case STATE_GOING_ON_ARC: // Going on the long drive towards the beacon.
                if (getLeftPos() >= LEFT_TARGET_DISTANCE * Constants.ENCODER_MULT && getRightPos() >= RIGHT_TARGET_DISTANCE * Constants.ENCODER_MULT) {
                    resetEncoders();
                    left.setPower(powah * 0.9);
                    right.setPower(powah * 0.9);
            ...
            case STATE_BACK_UP_LAUNCH_BALL_1B: // Backing up and launching ball.
                if ((getLeftPos() <= -780 * Constants.ENCODER_MULT) && (getRightPos() <= -780 * Constants.ENCODER_MULT)) {
                    left.setPower(0.0);
                    right.setPower(0.0);
            ...
            case STATE_LAUNCH_BALLS_1B: // Launch balls.
                if (System.currentTimeMillis() - timeBeforeFive >= 1900) { // Reverse chronological order.
                    bll.setPower(0);
                    blr.setPower(0);
                ...
                else if (System.currentTimeMillis() - timeBeforeFive >= 1400) {
                    s1.setPosition(Constants.SERVO_MAX_2016); // Launch second ball.
                }
                else if (System.currentTimeMillis() - timeBeforeFive >= 900) {
                    s2.setPosition(0.3); // Lower gate.
                }
                else if (System.currentTimeMillis() - timeBeforeFive >= 400) {
                    s1.setPosition(Constants.SERVO_MIN_2016); // Drop launch servo.
                }
                break;
            ...
            case STATE_BACK_UP_TURN_AWAY_1B: // Turning away from the beacon.
                if (getLeftPos() >= 690 * Constants.ENCODER_MULT && getRightPos() <= -690 * Constants.ENCODER_MULT) {
                    resetEncoders();
                    left.setPower(powah * 1.7);
                    right.setPower(powah * 1.7);
                    state = State.STATE_DRIVE_TO_NEXT_BEACON_2B;
                }
                break;
            case STATE_DRIVE_TO_NEXT_BEACON_2B: // Driving towards the next beacon.
                if ((getLeftPos() >= 2580 * Constants.ENCODER_MULT) && (getRightPos() >= 2580 * Constants.ENCODER_MULT)) {
                    resetEncoders();
                    left.setPower(-powah * 0.8);
                    right.setPower(powah * 0.8);
                    state = State.STATE_TURN_TO_FACE_BEACON_2B;
                }
                break;
            case STATE_TURN_TO_FACE_BEACON_2B: // Turns towards second beacon.
                if (getLeftPos() <= -630 * Constants.ENCODER_MULT && getRightPos() >= 630 * Constants.ENCODER_MULT) {
                    resetEncoders();
                    if (System.currentTimeMillis() - beginningTime >= 22000) {
                        left.setPower(powah * 1.1);
                        right.setPower(-powah * 1.1);
                        state = State.STATE_TURN_TO_HIT_BALL;
                    }
                    else {
                        left.setPower(powah * 0.9);
                        right.setPower(powah * 0.9);
                ...
                break;
            case STATE_TURN_TO_HIT_BALL: // Turn to go hit the ball.
                if (getLeftPos() >= 250 && getRightPos() <= -250) {
                    resetEncoders();
                    left.setPower(-powah * 1.7);
                    right.setPower(-powah * 1.7);
                    state = State.STATE_GO_HIT_BALL;
                }
                break;
            case STATE_GO_HIT_BALL: // Go hit the ball.
                if (getLeftPos() <= -2300 && getRightPos() <= -2300) {
                    resetEncoders();
                    left.setPower(0);
                    right.setPower(0);
