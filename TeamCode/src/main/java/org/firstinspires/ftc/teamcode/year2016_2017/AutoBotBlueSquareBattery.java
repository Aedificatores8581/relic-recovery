...
@Autonomous(name = "Autonomous Blue [Battery]", group = "Autonomous")
@Disabled
public class AutoBotBlueSquareBattery extends OpModeCamera {
...
// Touch sensor is "ts1".
    ...
    double configTurnPower = 4.2;

    Gamepad prev1;

    public static final double LEFT_SPEED_FACTOR = (67.0 / 51.0);
    public static final double TICKS_FROM_INCHES = 3700.0 / 70.0;
    public static final int RIGHT_TARGET_DISTANCE = (int)(TICKS_FROM_INCHES * 55.0 * Math.PI / 2.0) - 1000;
    ...
    public void loop() {
        ...
        switch (state) {
            case STATE_FIRST: // Initialization
                ...
                configArcFactor = 3.597 - (vs.getVoltage() / 7.0);
            ...
            case STATE_CHECK_COLOR_1B: // Checks color.
                if (System.currentTimeMillis() - timeBeforeFive >= 2100) {
                    color = getColor();
                    if (color == 2) { // Target color is RED. Move on.
                        resetEncoders();
            ...
            case STATE_BACK_UP_TURN_AWAY_1B: // Turning away from the beacon.
                if (/*getLeftPos() >= 700 * Constants.ENCODER_MULT ||*/ getRightPos() >= 480 * Constants.ENCODER_MULT * 1.2) {
                    resetEncoders();
            ...
            case STATE_TURN_TO_FACE_BEACON_2B: // Turns towards second beacon.
                if (/*getLeftPos() >= 850 * Constants.ENCODER_MULT ||*/ getRightPos() >= 800 * Constants.ENCODER_MULT * 1.2) {
                    left.setPower(0);
                    right.setPower(0);
                    resetEncoders();
        ...
        */
        ...
        telemetry.addData("Arc Factor", configArcFactor);
        /*
