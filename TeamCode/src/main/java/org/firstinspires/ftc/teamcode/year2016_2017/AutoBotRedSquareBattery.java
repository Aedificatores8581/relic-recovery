...
@Autonomous(name = "Autonomous Red [Battery]", group = "Autonomous")
@Disabled
public class AutoBotRedSquareBattery extends OpModeCamera {
...
// Touch sensor is "ts1".
    ...
    double ballLaunchSpeed = 0.25 * Constants.BALL_LAUNCHER_DIR_2016;
    ...
    public static final int RIGHT_TARGET_DISTANCE = (int)(TICKS_FROM_INCHES * 55.0 * Math.PI / 2.0) - 300;
    ...
    public void loop() {
        ...
        switch (state) {
            case STATE_FIRST: // Initialization
                ...
                configArcFactor = (vs.getVoltage() * (1.45 - 1.75)) + (2.416);
            ...
            case STATE_FORWARDS_BEFORE_ARC: // Going forwards.
                if ((getLeftPos() >= 50 * Constants.ENCODER_MULT * 1.2) && (getRightPos() >= 50 * Constants.ENCODER_MULT * 1.2)) {
                    resetEncoders();
                    left.setPower(powah * 2.0 / (LEFT_SPEED_FACTOR * configArcFactor));
            ...
            case STATE_GOING_ON_ARC: // Going on the long drive towards the beacon.
                ...
                int rightPos = getLeftPos();
                if (rightPos >= RIGHT_TARGET_DISTANCE * Constants.ENCODER_MULT * 1.2) {
                    resetEncoders();
                    left.setPower(0);
                    right.setPower(0);
            ...
            case STATE_BACK_UP_TURN_AWAY_1B: // Turning away from the beacon.
                if (/*getLeftPos() >= 700 * Constants.ENCODER_MULT ||*/ getRightPos() >= 800 * Constants.ENCODER_MULT * 1.2) {
                    resetEncoders();
            ...
            case STATE_DRIVE_TO_NEXT_BEACON_2B: // Driving towards the next beacon.
                if ((getLeftPos() >= 1650 * Constants.ENCODER_MULT * 1.2) && (getRightPos() >= 1650 * Constants.ENCODER_MULT * 1.2)) {
                    resetEncoders();
            ...
            case STATE_TURN_TO_FACE_BEACON_2B: // Turns towards second beacon.
                if (/*getLeftPos() >= 850 * Constants.ENCODER_MULT ||*/ getRightPos() >= 450 * Constants.ENCODER_MULT * 1.2) {
                    left.setPower(0);
                    right.setPower(0);
                    resetEncoders();
            ...
            case STATE_TURN_TO_HIT_BALL: // Turn to go hit the ball.
                if (/*getLeftPos() >= 220 ||*/ getRightPos() >= 300 * 1.2) {
                    resetEncoders();
            ...
            case STATE_GO_HIT_BALL: // Go hit the ball.
                if (getLeftPos() >= 1600 && getRightPos() >= 1600 * 1.2) {
                    resetEncoders();
                    left.setPower(0);
                    right.setPower(0);
            ...
            case STATE_BACK_UP_FROM_BEACON_2B: // Backing up.
                if ((getLeftPos() >= 300 * Constants.ENCODER_MULT * 1.2) && (getRightPos() >= 300 * Constants.ENCODER_MULT * 1.2)) {
                    left.setPower(0.0);
                    right.setPower(0.0);
