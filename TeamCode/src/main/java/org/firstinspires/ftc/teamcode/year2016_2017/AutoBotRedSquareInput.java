...
@Autonomous(name = "Autonomous Red [Input]", group = "Autonomous")
//@Disabled
public class AutoBotRedSquareInput extends OpModeCamera {
...
// Touch sensor is "ts1".
    ...
    int turnFromFirstBeacon = 640;
    int turnToSecondBeacon = 640;

    long beginningTime, runningTime;
    int color;

    double configArcFactor = 1.42;
    double configTurnPower = 1.75;
        ...
        switch (state) {
            case STATE_FIRST: // Initialization
                ...
                if (vs.getVoltage() > 13.5)
                    ballLaunchSpeed = 0.31 * Constants.BALL_LAUNCHER_DIR_2016;
                else if (vs.getVoltage() > 13.2)
                    ballLaunchSpeed = 0.32 * Constants.BALL_LAUNCHER_DIR_2016;
                else
                    ballLaunchSpeed = 0.33 * Constants.BALL_LAUNCHER_DIR_2016;
            ...
            case STATE_DRIVE_TO_NEXT_BEACON_2B: // Driving towards the next beacon.
                if ((getLeftPos() >= 2120 * Constants.ENCODER_MULT * 1.2) && (getRightPos() >= 2120 * Constants.ENCODER_MULT * 1.2)) {
                    resetEncoders();
            ...
            case STATE_TURN_TO_HIT_BALL: // Turn to go hit the ball.
                if (/*getLeftPos() >= 220 ||*/ getRightPos() >= 220 * 1.2) {
                    resetEncoders();
            ...
            case STATE_GO_HIT_BALL: // Go hit the ball.
                if (getLeftPos() >= 1800 && getRightPos() >= 1800 * 1.2) {
                    resetEncoders();
                    left.setPower(0);
                    right.setPower(0);
            ...
            case STATE_CHECK_SQUARING_2B:
                if (System.currentTimeMillis() - timeDuringSquare >= 250) {
                    if (Math.abs(uss2.getUltrasonicLevel() - uss1.getUltrasonicLevel()) <= 1.0)
