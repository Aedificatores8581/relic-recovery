...
@Autonomous(name = "Autonomous Red [L]", group = "Autonomous")
@Disabled
public class AutoBotRedSquareLinear extends OpModeCamera {
...
// Touch sensor is "ts1".
    ...
    public static final double LEFT_SPEED_FACTOR = 1.46 * (67.0 / 51.0);
    //public static final double LEFT_SPEED_FACTOR = 2.0 * (67.0 / 51.0);
    public static final double TICKS_FROM_INCHES = 3700.0 / 70.0;
    public static final int RIGHT_TARGET_DISTANCE = (int)(TICKS_FROM_INCHES * 55.0 * Math.PI / 2.0) - 800;
    ...
    */
    ...
    private int getColor() {
        ...
        for (int x = 31; x <= 40; x++) {
            for (int y = rgbImage.getHeight() / 3; y < rgbImage.getHeight() * 2 / 3; y++) {
    ...
    public void loop() {
        ...
        switch (state) {
            case STATE_FIRST: // Initialization
                ...
                //right.setPower(powah * 2.0 / LEFT_SPEED_FACTOR);
                left.setPower(-0.44);
                right.setPower(-0.44 / LEFT_SPEED_FACTOR);
            ...
            case STATE_GOING_ON_ARC: // Going on the long drive towards the beacon.
                ...
                if (leftPos >= RIGHT_TARGET_DISTANCE * Constants.ENCODER_MULT * 1.2) {
                    resetEncoders();
                    left.setPower(0);
                    right.setPower(0);
            ...
            case STATE_BACK_UP_LAUNCH_BALL_1B: // Backing up and launching ball.
                if ((getLeftPos() >= 150 * Constants.ENCODER_MULT * 1.2) && (getRightPos() >= 150 * Constants.ENCODER_MULT * 1.2)) {
                    left.setPower(0.0);
                    right.setPower(0.0);
            ...
            case STATE_CHECK_COLOR_1B: // Checks color.
                if (System.currentTimeMillis() - timeBeforeFive >= 2100) {
                    ...
                    if (color == 0) { // Target color is RED. Move on.
                        resetEncoders();
                        left.setPower(-powah * 1.75);
                        right.setPower(powah * 1.75);
            ...
            case STATE_SET_POW_TO_DRIVE_FWD_1B: // Set motor powers to drive forwards.
                if (System.currentTimeMillis() - timeBeforeFive >= 5750 * 1.2) {
                    left.setPower(Constants.POW_TO_BEACON_2016);
                    right.setPower(Constants.POW_TO_BEACON_2016);
            ...
            case STATE_BACK_UP_TURN_AWAY_1B: // Turning away from the beacon.
                if (/*getLeftPos() >= 700 * Constants.ENCODER_MULT ||*/ getRightPos() >= 660 * Constants.ENCODER_MULT * 1.2) {
                    resetEncoders();
            ...
            case STATE_TURN_TO_FACE_BEACON_2B: // Turns towards second beacon.
                if (/*getLeftPos() >= 850 * Constants.ENCODER_MULT ||*/ getRightPos() >= 600 * Constants.ENCODER_MULT) {
                    left.setPower(0);
                    right.setPower(0);
                    resetEncoders();
            ...
            case STATE_TURN_TO_HIT_BALL: // Turn to go hit the ball.
                if (/*getLeftPos() >= 220 ||*/ getRightPos() >= 240 * 1.2) {
                    resetEncoders();
            ...
            case STATE_GO_HIT_BALL: // Go hit the ball.
                if (getLeftPos() >= 1600 * 1.2 && getRightPos() >= 1600 * 1.2) {
                    resetEncoders();
                    left.setPower(0);
                    right.setPower(0);
