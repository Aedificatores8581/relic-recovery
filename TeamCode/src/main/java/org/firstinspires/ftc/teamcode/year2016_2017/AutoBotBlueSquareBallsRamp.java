...
@Autonomous(name = "Autonomous Blue Balls/Ramp", group = "Autonomous")
@Disabled
public class AutoBotBlueSquareBallsRamp extends OpModeCamera {
    ...
    public void loop() {
        ...
        switch (state) {
            ...
            case STATE_TURN_TO_FACE_BEACON_2B: // Turns towards second beacon.
                if (getLeftPos() >= 750 * Constants.ENCODER_MULT * 1.2 || getRightPos() >= 750 * Constants.ENCODER_MULT * 1.2) {
                    left.setPower(0);
                    right.setPower(0);
                    resetEncoders();
            ...
            case STATE_TURN_TO_RAMP: // Turn to go hit the ball.
                if (getLeftPos() >= 490 * 1.2 || getRightPos() >= 490 * 1.2) {
                    resetEncoders();
