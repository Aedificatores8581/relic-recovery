...
@Autonomous(name = "Autonomous Blue [L]", group = "Autonomous")
@Disabled
public class AutoBotBlueSquareLinear extends OpModeCamera {
...
// Touch sensor is "ts1".
    ...
    public static final double LEFT_SPEED_FACTOR = 1.4 * (67.0 / 51.0);
    ...
    public void loop() {
        ...
        switch (state) {
            ...
            case STATE_BACK_UP_LAUNCH_BALL_1B: // Backing up and launching ball.
                if ((getLeftPos() >= 350 * Constants.ENCODER_MULT * 1.2) && (getRightPos() >= 350 * Constants.ENCODER_MULT * 1.2)) {
                    left.setPower(0.0);
                    right.setPower(0.0);
            ...
            case STATE_BACK_UP_TURN_AWAY_1B: // Turning away from the beacon.
                if (getLeftPos() >= 600 * Constants.ENCODER_MULT * 1.2 || getRightPos() >= 600 * Constants.ENCODER_MULT * 1.2) {
                    resetEncoders();
            ...
            case STATE_DRIVE_TO_NEXT_BEACON_2B: // Driving towards the next beacon.
                if ((getLeftPos() >= 1575 * Constants.ENCODER_MULT * 1.2) && (getRightPos() >= 1575 * Constants.ENCODER_MULT * 1.2)) {
                    resetEncoders();
                    left.setPower(-powah * 1.75);
                    right.setPower(powah * 1.75);
            ...
            case STATE_TURN_TO_HIT_BALL: // Turn to go hit the ball.
                if (getLeftPos() >= 220 * 1.2 || getRightPos() >= 220 * 1.2) {
                    resetEncoders();
