...
@Autonomous(name = "[RED] LAUNCH BALLS AND RAMP", group = "Autonomous")
@Disabled
public class AutoBotLaunchBallsRed extends OpMode {
    ...
    public void loop() {
        ...
        switch (state) {
            ...
            case STATE_MOVE_AWAY_FROM_START:
                if ((getLeftPos() >= 450 * Constants.ENCODER_MULT) && (getRightPos() >= 450 * Constants.ENCODER_MULT)) {
                    resetEncoders();
                    left.setPower(powah * 0.75);
                    right.setPower(-powah * 0.75);
            ...
            case STATE_DRIVE_TO_RAMP:
                if (getLeftPos() >= 2750 && getRightPos() >= 2750) {
                    left.setPower(0.0);
                    right.setPower(0.0);
