...
@Disabled
public class AutoBotBlueSquare extends OpModeCamera {
...
// Touch sensor is "ts1".
    ...
    */
    ...
    private int getColor() {
        ...
        for (int x = rgbImage.getWidth() / 6; x < rgbImage.getWidth() * 5 / 6; x++) {
    ...
    */
    ...
    public void loop() {
        ...
        switch (state) {
            case 2: // Going forwards.
                if ((getLeftPos() >= 150 * Constants.ENCODER_MULT) && (getRightPos() >= 150 * Constants.ENCODER_MULT)) {
                    resetEncoders();
            ...
            case 3: // Going on the long drive towards the beacon. This has the robot ramp down its power.
                if (getLeftPos() >= LEFT_TARGET_DISTANCE * Constants.ENCODER_MULT && getRightPos() >= RIGHT_TARGET_DISTANCE * Constants.ENCODER_MULT) {
                    ...
                    double power = 0.3;
                    left.setPower(-power / LEFT_SPEED_FACTOR);
            ...
            case 11: // Driving towards the next beacon.
                if ((getLeftPos() >= 2200 * Constants.ENCODER_MULT) && (getRightPos() >= 2200 * Constants.ENCODER_MULT)) {
                    resetEncoders();
            ...
            case 12: // Turns towards second beacon.
                if (getLeftPos() <= -550 * Constants.ENCODER_MULT && getRightPos() >= 550 * Constants.ENCODER_MULT) {
                    resetEncoders();
            ...
            case 15: // Squares up with the wall.
                ...
                break;
            case 16: // Makes sure that the ultrasonic values are correct. Used by state 5.
